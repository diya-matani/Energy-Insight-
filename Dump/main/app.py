...
def load_models():
    try:
        ...
        clf_appliances = joblib.load('appliances.joblib')
        return clf_heat, clf_cool, clf_appliances
    except FileNotFoundError:
        st.error("Model files not found. Please run 'retrain_models.py' first.")
...
def main():
    ...
    tab_home, tab_weather, tab_load, tab_appliances, tab_ai = st.tabs([
        "🏠 Home", "☁️ Weather", "🏗️ Building Load", "🔌 Appliances", "🤖 AI Expert"
    ])
    ...
    # --- WEATHER TAB ---
    with tab_weather:
        st.markdown("### ☁️ Real-Time Weather Data")
    ...
    # --- BUILDING LOAD TAB ---
    with tab_load:
        st.markdown("### 🏗️ Predict Heating & Cooling Loads")
        ...
        with st.form("load_form"):
            ...
            if submitted:
                ...
                res2.metric("❄️ Cooling Load", f"{cool_pred:.2f} kWh")
    ...
    with tab_appliances:
        ...
        with st.form("appliance_form"):
            ...
            with st.expander("🌦️ Outdoor & Weather"):
                c1, c2, c3 = st.columns(3)
